...
const PRE_COMMIT_HOOK: &str =
    "#!/bin/sh\nset -eu\nleft_pocket sync-registry-git >/dev/null\ngit add -A .\n";
...
pub fn registry_dir() -> Result<PathBuf> {
    ...
    fs::create_dir_all(&dir).context("Failed to create left_pocket registry directory")?;
...
pub fn current_registry_dir() -> Result<PathBuf> {
    ...
    fs::create_dir_all(&dir).context("Failed to create primary left_pocket registry directory")?;
    ensure_registry_git_state_from(&dir)?;
    Ok(dir)
}
...
pub fn temporary_registry_dir() -> Result<PathBuf> {
    ...
    fs::create_dir_all(&dir).context("Failed to create temporary left_pocket directory")?;
    Ok(dir)
}
...
pub fn load_cache_or_rebuild() -> Result<RegistryCache> {
    ...
    // Split-brain protection: when the same left_pocket hash (directory name) exists
    // in multiple registry roots (e.g. ~/.left_pocket AND ~/.safe_pocket), pick the
...
/// Pick a single canonical entry per left_pocket hash (directory name).
///
...
///      (i.e. the left_pocket that "owns" this hash).
...
fn pick_dedupe_survivor(
    ...
    // 1. Most user content. When the same left_pocket hash lives in multiple roots,
    //    the left_pocket with more real user content (FEATURES, observations, tools,
    //    etc.) is almost certainly the "real" working left_pocket. A freshly created
...
/// Count user-content files in a left_pocket directory.
///
/// Walks the left_pocket recursively but EXCLUDES:
...
///   - Directories that contain a `manifest.json` (nested left_pocket copies)
///   - `.DS_Store` files (macOS filesystem metadata)
///
/// This is the split-brain tiebreaker: the left_pocket with more user content
/// (FEATURES, observations, tools, graphify-out, etc.) is the "real" working
/// left_pocket, while an accidentally created duplicate will have near-empty
/// content directories.
///
...
fn count_left_pocket_content_recursive(dir: &Path, count: &mut usize) {
    ...
    for entry in entries.flatten() {
        ...
        if file_type.is_dir() {
            ...
            // Skip nested left_pocket copies: a directory that contains a
            // manifest.json is itself a left_pocket, not user content.
...
/// reads each left_pocket's manifest directly. Used as a fallback when the cache
...
pub fn upsert_left_pocket(left_pocket_dir: &Path, manifest: &Manifest) -> Result<()> {
    ...
    // Split-brain protection: when this left_pocket's hash (directory name) also
...
pub fn move_to_unhoused(path: &Path, operation: &str) -> Result<Option<PathBuf>> {
    let root = crate::branding::known_registry_roots()?
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "Refusing to move path outside known left_pocket roots: {}",
                path.display()
            )
        })?;
    if !path.exists() {
    ...
    fs::rename(path, &target).with_context(|| {
        format!(
            "Failed to move left_pocket content to unhoused: {} -> {}",
            path.display(),
            target.display()
        )
    })?;
...
///
/// Parsing JSON or launching `left_pocket` on every prompt redraw is needlessly
...
/// leave one code path treating it as a left_pocket.
...
/// True when `name` is a reserved registry directory rather than a left_pocket.
...
fn ensure_registry_git_state_from(root: &Path) -> Result<()> {
    fs::create_dir_all(root).context("Failed to create left_pocket registry root")?;
...
fn sync_registry_snapshot_from(root: &Path) -> Result<usize> {
    ...
    for entry in fs::read_dir(root).context("Failed to read left_pocket registry root")? {
        let entry = entry?;
        let path = entry.path();
        if !path.is_dir()
...
fn copy_file_for_snapshot(src: &Path, dst: &Path) -> Result<()> {
    ...
    if metadata.len() as usize <= SNAPSHOT_CHUNK_SIZE {
        fs::copy(src, dst).with_context(|| {
            format!(
                "Failed to copy left_pocket content into snapshot: {} -> {}",
                src.display(),
                dst.display()
            )
        })?;
...
mod tests {
    ...
    fn count_left_pocket_content_counts_user_files() {
        ...
        fs::create_dir_all(left_pocket.join("FEATURES/dailies")).unwrap();
        fs::create_dir_all(left_pocket.join("observations")).unwrap();
        fs::create_dir_all(left_pocket.join(".opencode")).unwrap();
        fs::write(left_pocket.join("FEATURES/dailies/day1.md"), "x").unwrap();
        fs::write(left_pocket.join("FEATURES/dailies/day2.md"), "x").unwrap();
        fs::write(left_pocket.join("observations/note.md"), "x").unwrap();
        fs::write(left_pocket.join("README.md"), "x").unwrap();
        // .opencode files must NOT count.
        fs::write(left_pocket.join(".opencode/agent.md"), "x").unwrap();
    ...
    fn count_left_pocket_content_excludes_nested_left_pocket_dirs() {
        ...
        let _ = fs::remove_dir_all(&root);
        let left_pocket = root.join("abc123");
        fs::create_dir_all(left_pocket.join("FEATURES")).unwrap();
        fs::write(left_pocket.join("FEATURES/real.md"), "x").unwrap();

        // Nested left_pocket — must NOT count.
        let nested = left_pocket.join("nestedleft_pocket");
        fs::create_dir_all(&nested).unwrap();
        fs::write(nested.join("manifest.json"), "{}").unwrap();
