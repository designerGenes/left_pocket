//! Post-installation operational tests for the currently running Corner binary.
//!
//! The harness deliberately invokes `std::env::current_exe()` for every tested
//! operation. It does not call Corner's internal workspace/template functions,
...
const RUNTIME_START: &str = "#CORNER_RUNTIME_CONTENT_START";
const RUNTIME_END: &str = "#CORNER_RUNTIME_CONTENT_END";
const PLACED_EDIT: &str = "# corner-real-world-placed-edit";
...
impl TestEnvironment {
    fn create(keep: bool) -> Result<Self> {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("corner-real-world-{}-{unique}", std::process::id()));
    ...
    fn config_templates(&self) -> PathBuf {
        self.home.join(".config/corner/templates")
    }
}
...
impl Drop for TestEnvironment {
    fn drop(&mut self) {
        if self.keep {
            ...
            // user's project, config root, or existing corner.
...
pub fn run(options: Options) -> Result<()> {
    let binary = std::env::current_exe().context("failed to locate the running Corner binary")?;
    ...
    let included_corner = if let Some(project) = included_project.as_deref() {
    ...
    suite.skip(
        "Bulk clean commands",
        "corner clean --all / --hard are intentionally never invoked by the harness",
    );
    suite.skip(
...
fn run_existing_project_checks(suite: &mut Suite, project: &Path) -> Result<Option<PathBuf>> {
    ...
    let corner = optional_corner_from_locate(&locate)?;

    if let Some(corner) = corner.as_deref() {
        let backup = backup_existing_corner(corner, project)?;
        println!("Retained complete content backup: {}", backup.display());
        suite.case("Existing corner backup", |_| {
            if !backup.join("BACKUP-MANIFEST.txt").is_file() {
                bail!("backup manifest was not written");
            }
            if hash_tree(corner)? != hash_tree(&backup.join("corner"))? {
                bail!("retained backup file-content hash does not match the existing corner");
            }
            Ok(format!(
        ...
        suite.skip(
            "Existing corner backup",
            "the supplied project has no registered corner; no corner required backing up",
        );
    }
    ...
    suite.case("Existing project locate is read-only", |suite| {
        ...
        let located = optional_corner_from_locate(&output)?;
        if located != corner {
    ...
    suite.case("Existing .opencode ownership audit", |_| {
        let Some(corner) = corner.as_deref() else {
            return Ok(
                "no registered corner, so there is no corner-local .opencode to audit".to_string(),
            );
        };
        let opencode = corner.join(".opencode");
        if !opencode.exists() {
        ...
        Ok(format!(
            "{} total (Corner-owned agent files: {}; external OpenCode/npm node_modules: {}). Nothing removed",
            human_bytes(total),
            human_bytes(agents),
            human_bytes(npm)
        ))
    ...
    suite.case("Literal template-variable artifact audit", |_| {
        ...
        if artifacts.is_empty() {
            Ok(
                "no active literal {{SPOCKET_CONFIG_ROOT}}/{{CORNER_CONFIG_ROOT}} directories found (snapshots/unhoused excluded)"
                    .to_string(),
            )
        } else {
            Ok(format!(
...
fn run_isolated_checks(
    ...
    seed_corner: Option<&Path>,
    all: bool,
) -> Result<()> {
    let project = env.project_from_seed("primary", seed_project)?;
    let seeded = match seed_corner {
        Some(seed_corner) => Some(seed_existing_corner(env, seed_corner, &project)?),
        None => None,
    };
    ...
    suite.case("Install default assets into isolated HOME", |suite| {
        ...
        for required in [
            env.home.join(".config/corner/templates/AGENTS.md"),
            env.home.join(".config/corner/templates/project.env.md"),
            env.home.join(".config/corner/directory_structure.yaml"),
        ] {
            if !required.is_file() {
    ...
    suite.case("Create a normal corner with corner -i", |suite| {
        suite.exec(
            env,
            &project,
            &["-i", project.to_string_lossy().as_ref(), "--silent"],
        )?;
        let corner = locate_corner(suite, env, &project)?;
        if !corner.join("manifest.json").is_file() {
            bail!("corner has no manifest: {}", corner.display());
        }
        Ok(format!("created {}", corner.display()))
    });

    if let Some(seeded) = seeded.as_deref() {
        // Without this check the seeding step can fail silently: an orphaned
        // clone leaves `corner -i` free to create a blank corner, and every
        // check below still passes while testing none of the placed state the
        // seeding exists to provide.
        suite.case("Seeded existing corner is adopted, not replaced", |suite| {
            let corner = locate_corner(suite, env, &project)?;
            if corner != seeded {
                bail!(
                    "corner -i created {} instead of adopting the seeded corner {}",
                    corner.display(),
                    seeded.display()
                );
            }
            let corners = fs::read_dir(env.home.join(".corner"))?
                .filter_map(Result::ok)
                .filter(|entry| entry.path().join("manifest.json").is_file())
                .count();
            if corners != 1 {
                bail!("expected exactly one corner in the isolated registry, found {corners}");
            }
            Ok(format!(
                "the copied corner was re-keyed and reused as existing placed state at {}",
                seeded.display()
            ))
        });
    }

    suite.case("Environment root placement is Corner-only", |suite| {
        let corner = locate_corner(suite, env, &project)?;
        for file in [project.join(".env"), corner.join(".env")] {
            ...
            // The Spocket/Safe_pocket rename is complete: Corner must no longer
        ...
        Ok("project and corner .env define PROJECT_ROOT and CORNER_ROOT, with no legacy SPOCKET_ROOT".to_string())
    });
    ...
    suite.case("Runtime merge start/stop round trip", |suite| {
        let corner = locate_corner(suite, env, &project)?;
        let agents = corner.join("AGENTS.md");
        ...
        if !started.contains(RUNTIME_START) || !started.contains(RUNTIME_END) {
            bail!("CORNER runtime markers were not injected on start");
        }
    ...
    suite.case("corner -u upgrade semantics", |suite| {
        let corner = locate_corner(suite, env, &project)?;
        let prompt = corner.join(".github/prompts/TalkLikeACat.md");
    ...
    if all {
        suite.case("Augment add/remove and idempotency", |suite| {
            ...
            let corner = locate_corner(suite, env, &project)?;
            if !manifest_contains_path(&corner, &sidecar)? {
        ...
        suite.case("Heal moves a corner to a new project", |suite| {
            ...
            let source_corner = locate_corner(suite, env, &source)?;
            suite.exec(
                env,
                &target,
                &[
            ...
            let healed = locate_corner(suite, env, &target)?;
            if !manifest_contains_path(&healed, &target)? {
                bail!("healed corner does not reference target project");
            }
            let old = suite.exec(
                env,
                &source,
            ...
            Ok(format!(
                "healed corner now resolves at {}",
                healed.display()
            ))
        });

        suite.case("Per-corner OpenCode agent placement", |suite| {
            let corner = locate_corner(suite, env, &project)?;
            suite.exec(env, &project, &["sync", "agents"])?;
            let agent_dir = corner.join(".opencode/agent");
            let count = fs::read_dir(&agent_dir)?.filter_map(Result::ok).count();
            if count == 0 {
                bail!("no OpenCode agents were rendered");
            }
            if corner.join(".opencode/node_modules").exists() {
                bail!("Corner unexpectedly installed node_modules into .opencode");
            }
            Ok(format!(
                "rendered {count} agent files; no node_modules were installed by Corner"
            ))
        });
        ...
        suite.case("No literal config-root directories are created", |_| {
            let mut bad = Vec::new();
            collect_named_dirs(
                &env.home.join(".corner"),
                &["{{SPOCKET_CONFIG_ROOT}}", "{{CORNER_CONFIG_ROOT}}"],
                &mut bad,
            )?;
            if !bad.is_empty() {
                bail!(
...
fn locate_corner(suite: &Suite, env: &TestEnvironment, project: &Path) -> Result<PathBuf> {
    let output = suite.exec(
        env,
        project,
        &["locate", "--path", project.to_string_lossy().as_ref()],
    )?;
    corner_from_locate(&output)
}

fn corner_from_locate(output: &Output) -> Result<PathBuf> {
    optional_corner_from_locate(output)?.ok_or_else(|| anyhow!("corner locate returned not_found"))
}

fn optional_corner_from_locate(output: &Output) -> Result<Option<PathBuf>> {
    ...
    match json.get("status").and_then(Value::as_str) {
        ...
        Some("found") => {}
        _ => bail!("corner locate returned {}", json),
    }
    let path = json
        .get("corner_dir")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("locate output has no corner_dir"))?;
    Ok(Some(PathBuf::from(path)))
}

fn manifest_contains_path(corner: &Path, expected: &Path) -> Result<bool> {
    let manifest: Value = serde_json::from_str(&fs::read_to_string(corner.join("manifest.json"))?)?;
...
/// Copy an existing corner into the isolated registry and re-key it to the
/// fixture project.
///
/// A corner is addressed by `hash_paths(core_paths)`: the directory name, the
/// `.code-workspace` filename and `manifest.hash` all have to agree, and
/// `Workspace::find_workspace_by_manifest_paths` only adopts a corner whose
...
/// ever resolves: `corner -i` would quietly create a second, blank corner and
/// every idempotency check below would run against empty state while still
/// reporting PASS. Re-keying is what makes the seeded corner real.
fn seed_existing_corner(
    env: &TestEnvironment,
    source_corner: &Path,
    ...
    let destination = env.home.join(".corner").join(&hash);
    copy_isolated_corner_clone(source_corner, &destination)?;

    let manifest_path = destination.join("manifest.json");
    let mut manifest: Value =
        serde_json::from_str(&fs::read_to_string(&manifest_path).with_context(|| {
            format!(
                "seeded corner has no readable manifest: {}",
    ...
    // from what `corner -i` would itself write would make the very first
    ...
    // could never point at the real corner; restore them against the *fixture*
    // paths here. Without this the adopted-corner path looks like a missing
    // placement, because `corner -i` deliberately does not re-place templates
    // for a corner that already exists.
...
fn backup_existing_corner(corner: &Path, project: &Path) -> Result<PathBuf> {
    let parent = corner
        .parent()
        .ok_or_else(|| anyhow!("corner has no registry parent: {}", corner.display()))?;
    ...
    fs::write(
        ...
        format!(
            "Created: {}\nCorner: {}\nProject: {}\nBackup policy: complete corner copy; no corner content excluded\n",
            Utc::now().to_rfc3339(),
            corner.display(),
            project.display()
        ),
...
/// Directory names never copied into the throwaway isolated corner clone.
///
/// `node_modules` is external OpenCode/npm content that Corner does not own or
/// regenerate and that routinely runs to tens of megabytes, so copying it into
/// a fixture on every `corner tests -i` run costs a great deal and proves
...
/// Copy a corner into the isolated fixture, dropping symlinks so no write can
/// escape back into the original tree.
fn copy_isolated_corner_clone(source: &Path, target: &Path) -> Result<()> {
    if !source.is_dir() {
        return Ok(());
    }
    fs::create_dir_all(target)?;
    for entry in fs::read_dir(source)? {
        ...
        if metadata.is_dir() {
            ...
            copy_isolated_corner_clone(&path, &destination)?;
        } else if metadata.is_file() {
            fs::copy(&path, &destination)?;
        }
    }
...
fn current_config_templates() -> Result<PathBuf> {
    ...
    for candidate in [
        home.join(".config/corner/templates"),
...
fn collect_named_dirs(root: &Path, names: &[&str], out: &mut Vec<PathBuf>) -> Result<()> {
    ...
    while let Some(dir) = stack.pop() {
        for entry in match fs::read_dir(&dir) {
            ...
            // The two retained backup trees hold verbatim copies of corner
...
mod tests {
    ...
    fn named_directory_scan_finds_literal_roots_and_skips_snapshots() {
        let root =
            std::env::temp_dir().join(format!("corner-real-world-scan-{}", std::process::id()));
    ...
    fn tree_hash_changes_with_content() {
        let root =
            std::env::temp_dir().join(format!("corner-real-world-hash-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
