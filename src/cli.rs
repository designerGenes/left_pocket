...
feature notes) in a dedicated pocket directory (~/.left_pocket/<hash>/) so they \
...
LOCKET DIRECTORY
  ...
  New pockets are stored in ~/.left_pocket/<hash>/. If that tree does not exist yet,
...
TEMPLATES
  ...
  (Legacy #LEFT_POCKET_TEMPLATE_DESTINATION, #CORNER_TEMPLATE_DESTINATION, and
  #SPOCKET_TEMPLATE_DESTINATION forms are still recognised when reading
  templates for backwards compatibility.)

  Supported variables: {{POCKET_ROOT}}/{{SPOCKET_ROOT}}, {{PROJECT_ROOT}},
  {{POCKET_NAME}}/{{SPOCKET_NAME}}, {{POCKET_CONFIG_ROOT}}/{{SPOCKET_CONFIG_ROOT}},
  {{POCKET_REGISTRY_ROOT}}/{{SPOCKET_REGISTRY_ROOT}}
...
pub struct Cli {
    ...
    /// Add a tool for this pocket session only
    ///
    ...
    /// Create or reuse the pocket under ~/.left_pocket/temporary/
    ///
    ...
    /// expands
    /// {{POCKET_ROOT}}/{{SPOCKET_ROOT}}, {{PROJECT_ROOT}}, and
    /// {{POCKET_NAME}}/{{SPOCKET_NAME}} variables, and
    ...
    ///
    /// PATH may be either:
    ///   • The pocket directory itself  (~/.left_pocket/abc123)
    ...
    /// #POCKET_RUNTIME_CONTENT_START / #POCKET_RUNTIME_CONTENT_END markers)
...
pub enum Commands {
    ...
    ///
    /// With a TARGET, synchronizes system-wide pocket assets that every
    /// pocket draws from:
    ///
    ...
    ///
    /// Copies every pocket into ~/.left_pocket/snapshots without nested
    ...
    ///
    /// Reads each pocket's on-disk manifest directly and rewrites
    ...
    /// single canonical entry per pocket hash. Use this after manually
    /// editing a manifest, moving pocket directories outside left_pocket, or when
    /// `left_pocket locate` / `left_pocket -i` resolve to the wrong pocket.
    ///
    ...
    ///
    /// For each template marked with `#POCKET_MERGE_AT_RUNTIME`, injects the expanded
    ...
    ///
    /// Removes any content between `#POCKET_RUNTIME_CONTENT_START` and
    /// `#POCKET_RUNTIME_CONTENT_END` markers from destination files.
    ...
    /// Rewrite legacy Corner/Spocket/left_pocket tokens to pocket tokens across installed roots
    ///
    ...
    /// ~/.spocket) for files that still contain legacy `#SPOCKET_*`,
    /// `#LEFT_POCKET_*`, or `#CORNER_*` directive
    /// or runtime markers, and rewrites them in place to their `#POCKET_*`
    /// equivalents.
    ///
    ...
    UpgradeInstallation {
        ...
        /// {{LEFT_POCKET_CONFIG_ROOT}} / {{POCKET_CONFIG_ROOT}} artifact
        /// directories, but only when they
    ...
    /// major pocket workflows, prints a detailed checklist, then removes only
    /// that harness-owned fixture. With `-i`, first backs up the existing pocket
    ...
    /// ~/.left_pocket/global_data/tasks.db and are grouped per pocket by a
...
pub enum WorktreeAction {
    ...
    /// already belongs to a pocket). left_pocket will also suggest any git worktrees
